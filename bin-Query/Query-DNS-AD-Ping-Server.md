...
if ($result) {# Create global VARS from DNS query
              $fqdn = $result.Name
              ...
              if (Get-Module -ListAvailable -Name ActiveDirectory) {# Query AD for object
                                                                    if ($servername) {Write-Host "`nQuerying for AD Record.." -ForegroundColor Yellow
